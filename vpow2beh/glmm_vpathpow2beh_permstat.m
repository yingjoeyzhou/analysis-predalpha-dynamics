...
% The number of bootstraps used
if ~exist('nPermute','var')
    nPermute = input('Please specify the number of permutations for each GLMM.\n', 's');
    nPermute = str2double( nPermute );
end
...
for iT = 1:numel(tvec)
    ...
    stat.Pval = [ stat.Pval; sout(1).Pvec ]; %stat.Pvals is an nTimes-by-nContrasts array
    stat.Fstat = [ stat.Fstat; sout(1).Fvec ]; %stat.Fvals is an nTimes-by-nContrasts array
    
    
    
    % ============== Do the fit for the permuted data ===============
    Dmat = nan(1,numel(dp_beta), nPermute); 
    Cmat = nan(1,numel(c_beta), nPermute); 
    Pmat = nan(1,size(stat.Pval,2), nPermute);
    ...
    for b = 1:nPermute
        ...
        Dmat(1,:,b) = dp_beta; %Dmat is an 1-by-2(ON/OFF)-by-nBS matrix
        Cmat(1,:,b) = c_beta; %Cmat is an 1-by-2(ON/OFF)-by-nBS matrix
        
        Pmat(1,:,b) = sout(1).Pvec; %Pmat is an 1-by-nContrasts-by-nBS array
        Fmat(1,:,b) = sout(1).Fvec; %Fmat is an 1-by-nContrasts-by-nBS array
    end
    
    betaBS.Dp = cat(1, betaBS.Dp, Dmat ); %statBS.dp is an nTimes-by-2(ON/OFF)-by-nBS matrix
    betaBS.C = cat(1, betaBS.C, Cmat ); %statBS.dp is an nTimes-by-2(ON/OFF)-by-nBS matrix
    
    statBS.Pval = cat(1, statBS.Pval, Pmat );
